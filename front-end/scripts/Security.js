...
class SecurityGuard extends Employee {
    constructor(param){
        ...
        // this.exp = this.exp + this.exp;  
    }

    // Fonction pour nettoyer la cage
    CleanCage(){
        if (this.level < 5){
        this.exp = this.exp + 100; //Guard gagne 1 d'exp
        return this.exp;
        }
    }

    // Fonction pour sauver un client // Garde de LVL 1
    ProtectClientLvl1(min,max,animal){
        ...
        if (chanceSave == 2 || chanceSave == 3) { //Probabilité d'avoir ce scénario
        ...
        if (chanceSave == 4) { //Probabilité d'avoir ce scénario
            // client.pv = 0; //Client mort
        ...
        if (this.exp >= 10 && chanceSave == 5) { //Probabilité d'avoir ce scénario
            // client.pv = 0; //Client mort
    ...
    ProtectClientLvl2(min,max,animal){
        min = Math.ceil(min); //Donne le minimum d'intervale
        max = Math.floor(max); //Donne le maximum d'intervale
        var chanceSave = Math.floor(Math.random() * (max - min + 1) + min); //Calcule un nombre aléatoire compris entre le minimum et le maximum donné juste avant
        // console.log("chanceSave: " + chanceSave);
        ...
        if (chanceSave == 3 || chanceSave == 4) { //Probabilité d'avoir ce scénario
            // client.pv = 0; //Client mort
            this.exp = this.exp + 3; //Gagne Expérience
            console.log("Malheureusement le client n'a pas survécu malgré l'intervention du garde de sécurité.");
            return this.exp;
    ...
    ProtectClientLvl3(min,max,animal){
        min = Math.ceil(min); //Donne le minimum d'intervale
        max = Math.floor(max); //Donne le maximum d'intervale
        var chanceSave = Math.floor(Math.random() * (max - min + 1) + min); //Calcule un nombre aléatoire compris entre le minimum et le maximum donné juste avant
        // console.log("chanceSave: " + chanceSave);
        ...
        if (chanceSave == 4 || chanceSave == 5) { //Probabilité d'avoir ce scénario
            // client.pv = 0; //Client mort
            this.exp = this.exp + 3; //Gagne Expérience
            console.log("Malheureusement le client n'a pas survécu malgré l'intervention du garde de sécurité.");
            return this.exp;
    ...
    ProtectClientLvl4(min,max,animal){
        min = Math.ceil(min); //Donne le minimum d'intervale
        max = Math.floor(max); //Donne le maximum d'intervale
        var chanceSave = Math.floor(Math.random() * (max - min + 1) + min); //Calcule un nombre aléatoire compris entre le minimum et le maximum donné juste avant
        // console.log("chanceSave: " + chanceSave);
